...
if __name__ == "__main__":
    ...
    for i in qubit_list:
        ...
        mcx_nancillary_log_depth(
            circuit1, list(range(i)), i, list(range(i + 1, i + 1 + i - 2))
        )
        transpile_cir1 = transpile(
            circuit1, basis_gates=["ecr", "id", "rz", "sx", "x"], optimization_level=3
        )
        log_depth_list.append(transpile_cir1.depth())
        print(i, transpile_cir1.num_nonlocal_gates())
        circuit2 = QuantumCircuit(i + 1 + i - 1)
        mcx_nancillary_linear_depth(
            circuit2, list(range(i)), i, list(range(i + 1, i + 1 + i - 1))
        )
        transpile_cir2 = transpile(
            circuit2, basis_gates=["ecr", "id", "rz", "sx", "x"], optimization_level=3
        )
